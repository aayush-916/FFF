...
import { 
  BookOpen, Layers, PlayCircle, ChevronRight, ArrowLeft, 
  Clock, AlertCircle, Download, Maximize2, Minimize2, ZoomIn, ZoomOut, FileText, GraduationCap, CheckCircle
} from 'lucide-react';
...
const LessonCard = ({ lesson, getFileUrl, onSubmitLesson }) => {
  ...
  const defaultTab = materials.length > 0 ? materials[0].id : (guidePath ? 'guide' : null);
  const [activeTab, setActiveTab] = useState(defaultTab);
  
  const [zoom, setZoom] = useState(1);
  const [isFullscreen, setIsFullscreen] = useState(false);

  useEffect(() => { setZoom(1); }, [activeTab]);
  ...
  if (activeTab === 'guide') {
    activeUrl = guidePath;
    activeDescription = 'Step-by-step instructions for the teacher to conduct this session.';
  } else {
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 flex flex-col w-full overflow-hidden">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 w-full hide-scrollbar">
          {materials.map((material) => (
            <button
              ...
              className={`shrink-0 whitespace-nowrap px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-1.5 ${
                activeTab === material.id ? 'bg-blue-600 text-white shadow-sm' : 'bg-gray-50 text-gray-600 hover:bg-gray-200'
              }`}
            >
          ...
          {guidePath && (
            <button
              ...
              className={`shrink-0 whitespace-nowrap px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-1.5 ${
                activeTab === 'guide' ? 'bg-orange-500 text-white shadow-sm' : 'bg-gray-50 text-gray-600 hover:bg-gray-200'
              }`}
            >
      ...
      {activeTab && activeUrl && (
        <div className={`flex flex-col bg-gray-900 shadow-inner transition-all duration-300 w-full mt-4 ${
          isFullscreen 
          ...
          <div className="flex items-center justify-between p-3 bg-gray-800 text-gray-200 shrink-0 z-10 border-b border-gray-700">
            <div className="flex items-center gap-1 bg-gray-900/50 rounded-lg p-1">
              <button onClick={handleZoomOut} className="p-1.5 hover:bg-gray-700 rounded-md transition-colors text-gray-400 hover:text-white">
                <ZoomOut className="w-5 h-5" />
              </button>
              <span className="text-xs font-bold w-12 text-center text-gray-300 select-none">
                {Math.round(zoom * 100)}%
              </span>
              <button onClick={handleZoomIn} className="p-1.5 hover:bg-gray-700 rounded-md transition-colors text-gray-400 hover:text-white">
                <ZoomIn className="w-5 h-5" />
              </button>
            </div>

            <div className="flex items-center gap-2">
              <a href={getFileUrl(activeUrl)} download className="p-2 hover:bg-gray-700 rounded-lg transition-colors text-gray-400 hover:text-white hidden sm:flex" title="Download Document">
                <Download className="w-5 h-5" />
              </a>
              <button onClick={() => setIsFullscreen(!isFullscreen)} className="px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors flex items-center gap-2 font-bold text-sm">
                {isFullscreen ? <><Minimize2 className="w-4 h-4" /> <span className="hidden sm:inline">Exit</span></> : <><Maximize2 className="w-4 h-4" /> <span className="hidden sm:inline">Fullscreen</span></>}
              </button>
            </div>
          </div>

          <div className="flex-1 overflow-auto relative bg-[#525659] flex justify-center items-start pt-4">
            <div className="origin-top transition-transform duration-200 ease-out shadow-lg" style={{ transform: `scale(${zoom})`, width: isFullscreen ? '100%' : '100%', height: isFullscreen ? '100%' : '95%' }}>
              <iframe src={`${getFileUrl(activeUrl)}#toolbar=0&navpanes=0&view=FitH`} className="w-full h-full border-0 bg-white" title="Document Viewer" />
            </div>
          </div>
        </div>
      )}
...
const LessonBrowser = () => {
  ...
  const fetchDomains = async () => {
    setLoadingDomains(true); setError(null);
    try {
      const response = await api.get('/domains');
      setDomains(response.data.data || response.data || []);
    } catch (err) { setError("Unable to load domains."); } 
    finally { setLoadingDomains(false); }
  };

  const handleDomainClick = async (domain) => {
    setSelectedDomain(domain); setLoadingHabits(true); setError(null);
    try {
      const response = await api.get(`/habits?domain_id=${domain.id}`);
      setHabits(response.data.data || response.data || []);
    } catch (err) { setError("Unable to load habits."); } 
    finally { setLoadingHabits(false); }
  };

  const handleHabitClick = async (habit) => {
    setSelectedHabit(habit); setLoadingLessons(true); setError(null);
    try {
      const response = await api.get(`/lessons?habit_id=${habit.id}`);
      setLessons(response.data.data || response.data || []);
    } catch (err) { setError("Unable to load lessons."); } 
    finally { setLoadingLessons(false); }
  };

  // --- Multi-Step Back Handlers ---
  const handleBackToDomains = () => {
    setSelectedDomain(null); setHabits([]); setError(null);
  };
  const handleBackToHabits = () => {
    setSelectedHabit(null); setLessons([]); 
    setSelectedClass(null); setIsClassSubmitted(false); setError(null);
  };
  ...
  const handleSubmitLesson = (lessonId) => {
    ...
    navigate(`/sessions/start?lesson_id=${lessonId}&class_num=${selectedClass}`);
  };
  ...
  return (
    <div className="max-w-md mx-auto md:max-w-3xl space-y-6 pb-6 w-full">
      ...
      <div className="flex items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        {backAction && (
          <button onClick={backAction} className="p-2.5 bg-gray-50 rounded-full text-gray-600 hover:bg-gray-200 transition-colors shrink-0">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight truncate">
            ...
            {selectedHabit && isClassSubmitted && `Class ${selectedClass} Lesson`}
          </h1>
          <p className="text-sm text-gray-500 mt-0.5 font-medium truncate">
            ...
            {selectedHabit && !isClassSubmitted && "Choose your class to load the correct materials."}
      ...
      {!selectedDomain && (
        <div>
          {loadingDomains ? <SkeletonList /> : domains.length > 0 ? (
            <div className="grid gap-4 w-full">
              {domains.map((domain) => (
                <button key={domain.id} onClick={() => handleDomainClick(domain)} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between hover:bg-blue-50 transition-all text-left group w-full">
                  <div className="flex items-center gap-4">
                    <div className="bg-blue-50 p-3.5 rounded-xl text-blue-600"><Layers className="w-6 h-6" /></div>
                    <div>
                      <h3 className="text-lg font-bold text-gray-900">{domain.name}</h3>
                      <p className="text-sm text-gray-500 font-medium">Tap to view habits</p>
                    </div>
                  </div>
          ...
          ) : <div className="text-center p-8 bg-white rounded-2xl shadow-sm text-gray-500 font-medium">No domains found.</div>}
        </div>
      )}
      ...
      {selectedDomain && !selectedHabit && (
        <div>
          {loadingHabits ? <SkeletonList /> : habits.length > 0 ? (
            <div className="grid gap-4 w-full">
              {habits.map((habit) => (
                <button key={habit.id} onClick={() => handleHabitClick(habit)} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between hover:bg-orange-50 transition-all text-left group w-full">
                  <div className="flex items-center gap-4">
                    <div className="bg-orange-50 p-3.5 rounded-xl text-orange-600"><BookOpen className="w-6 h-6" /></div>
                    <div>
                      <h3 className="font-bold text-gray-900 text-lg leading-tight">{habit.name}</h3>
                      <p className="text-sm text-gray-500 mt-1 line-clamp-2 font-medium">{habit.description}</p>
                    </div>
                  </div>
                  <ChevronRight className="text-gray-300 w-6 h-6 shrink-0" />
                </button>
          ...
          ) : <div className="text-center p-8 bg-white rounded-2xl shadow-sm text-gray-500 font-medium">No habits found.</div>}
        </div>
      )}
      ...
      {selectedHabit && !isClassSubmitted && (
        <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-6 w-full">
          <div className="text-center space-y-2">
            ...
            <h2 className="text-xl font-bold text-gray-900">Which class are you teaching?</h2>
            <p className="text-gray-500 text-sm">Classes 3-5 use Lower Grades materials. Classes 6-8 use Higher Grades materials.</p>
          </div>
      ...
      {selectedHabit && isClassSubmitted && (
        <div>
          {loadingLessons ? <SkeletonList /> : (() => {
            // Determine the target tier based on selected class
            const targetType = [3, 4, 5].includes(selectedClass) ? 'Lower' : 'Higher';
            
            // Find the lesson that matches the tier (case insensitive)
            const displayLesson = lessons.find(l => 
              l.type === targetType || 
            ...
            if (!displayLesson) {
              return (
                <div className="text-center p-10 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center">
                  ...
                  <p className="text-gray-500 font-medium">No {targetType} tier lesson found for this habit yet.</p>
                </div>
              );
            }
